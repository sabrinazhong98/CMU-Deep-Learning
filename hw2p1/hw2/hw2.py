...
class CNN(object):
    ...
    def __init__(self, input_width, num_input_channels, num_channels, kernel_sizes, strides,
       ...
       # print(activations)
        ...
        out_width =  ((input_width - kernel_sizes[0])//strides[0]) + 1
        self.convolutional_layers.append(Conv1D(num_input_channels, num_channels[0], kernel_sizes[0], strides[0], \
        ...
        for i in range(1, self.nlayers):
          #  print(num_channels[i])
            self.convolutional_layers.append(Conv1D(num_channels[i-1], num_channels[i], kernel_sizes[i], strides[i], \
                                                       weight_init_fn= conv_weight_init_fn, bias_init_fn = bias_init_fn))
            
           # print(input_width, kernel_sizes[i], strides[i])
            out_width =  ((input_width - kernel_sizes[i])//strides[i]) + 1
            input_width = out_width
    ...
    def backward(self, labels):
        ...
        for i in range(self.nlayers-1, -1,-1):
            dz_current = grad * self.activations[i].derivative() 
                
            grad = self.convolutional_layers[i].backward(dz_current)          
